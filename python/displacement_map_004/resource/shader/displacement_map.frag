...
vec3 SteepParallax( in float bottom_rel, in vec3 texDir3D, in vec2 texC0, in vec2 texC1 )
{
  ...
  vec2  texCoord      = mix(texC1, texC0, frontface);
  if ( maxBumpHeight > 0.0 && texDir3D.z < 0.9994 )
  {
    ...
    vec2  texStep         = (texC1 - texC0) * facesign;
...
void main()
{
    ...
    vec2  texCoords1  = inData.uv1;
    vec2  texCoords0  = inData.uv0;
    ...
    vec2  tc_dx        = dFdx( texCoords1 );
    vec2  tc_dy        = dFdy( texCoords1 );
    ...
    vec3  texCoordsMap = SteepParallax( inData.vsPos_rel01, texDir3D, texCoords0.st, texCoords1.st );
    vec4  normalVec    = CalculateNormal( texCoordsMap.st ); 
    ...
    vec3 color = texture( u_texture, texCoordsMap.st ).rgb;
