...
//#define VULKAN_DYNAMIC_LOADING

#if defined(VULKAN_DYNAMIC_LOADING)
...
struct TPhysicalDevice {
    vk::PhysicalDevice         _physical_device;   //!< device handle
...
private: // private operations
    ...
    int rateDeviceSuitability( vk::PhysicalDevice device, const std::set<std::string> &device_extensions, const TSwapChainSupportDetails &swapChainSupport );
    
    void                     deviceInformation( vk::PhysicalDevice device, std::ostream &stream );                                 //!< get the device information
    TDeviceQueueInfo         findQueueFamilies( vk::PhysicalDevice device );                                                       //!< find the queue families from an device
    ...
    TSwapChainSupportDetails querySwapChainSupport(vk::PhysicalDevice device);                                                     //!< populate swapchain details
    vk::SurfaceFormatKHR     chooseSwapSurfaceFormat( const std::vector<vk::SurfaceFormatKHR>& availableFormats );                 //!< choose the surface format
    vk::PresentModeKHR       chooseSwapPresentMode( const std::vector<vk::PresentModeKHR> availablePresentModes );                 //!< choose the presentation mode
    vk::Extent2D             chooseSwapExtent( const vk::SurfaceCapabilitiesKHR& capabilities );                                   //!< choose the swapchain 2D extent
    vk::ShaderModule         createShaderModule( const std::vector<char>& code );                                                  //!< create shader module from byte code
    uint32_t                 findMemoryType( vk::PhysicalDevice device, uint32_t typeFilter, vk::MemoryPropertyFlags properties ); //!< evaluate type of memory
    ...
    void createImageView(vk::Image image, vk::Format format, vk::ImageAspectFlags aspectFlags, uint32_t mipLevels, vk::ImageView &imageView);
...
private: // private attributes
    ...
    std::vector<vk::ExtensionProperties> _extensions;            //!< available vulkan extensions
    std::vector<std::string>             _extensionNames;        //!< available vulkan extension names
    std::vector<vk::LayerProperties>     _validationLayers;      //!< available vulkan validation layers
    ...
    vk::DebugReportCallbackEXT _debubCallback = nullptr;
    ...
    vk::SampleCountFlagBits _msaaSamples = vk::SampleCountFlagBits::e1; //!< samples for multisample antialiasing

    TPhysicalDevices               _physicalDevices;          //!< Vulkan physical device handles
    vk::Instance                   _instance;                 //!< Vulkan instance handle
    vk::SurfaceKHR                 _surface;                  //!< Vulkan surface handle
    vk::Device                     _device;                   //!< Vulkan logical device handle 
    vk::Queue                      _graphicsQueue;            //!< Vulkan graphics queue handle 
    vk::Queue                      _presentQueue;             //!< Vulkan presentation queue handle 
    vk::SwapchainKHR               _swapChain;                //!< Vulkan swap chain handle 
    std::vector<vk::Image>         _swapChainImages;          //!< Vulkan swap chain image handles
    vk::SurfaceFormatKHR           _surfaceFormat;            //!< selected surface format
    vk::PresentModeKHR             _presentMode = vk::PresentModeKHR::eImmediate; //!< selected presentation mode
    vk::Extent2D                   _extent;                   //!< selected 2D extent
    std::vector<vk::ImageView>     _swapChainImageViews;      //!< Vulkan swap chain image handles
    vk::RenderPass                 _renderPass;               //!< Vulkan render pass handle
    vk::DescriptorSetLayout        _descriptorSetLayout;      //!< Vulkan descriptor set layout
    ...
    vk::PipelineLayout             _pipelineLayout;           //!< Vulkan pipeline layout handle
    vk::Pipeline                   _graphicsPipeline;         //!< Vulkan graphics pipeline handle
    ...
    std::vector<vk::Buffer>        _vertexBuffers;            //!< Vulkan vertex buffer
    std::vector<vk::DeviceMemory>  _vertexBufferMemories;     //!< Vulkan memories used for vertex buffers
    std::vector<vk::Buffer>        _indexBuffers;             //!< Vulkan index buffer
    std::vector<vk::DeviceMemory>  _indexBufferMemories;      //!< Vulkan memories used for index buffers
    std::vector<vk::Buffer>        _uniformBuffers;           //!< Vulkan uniform buffer
    std::vector<vk::DeviceMemory>  _uniformBuffersMemories;   //!< Vulkan memories used for uniform buffers
    std::vector<vk::Image>         _depthImage;               //!< Vulkan depth image 
    std::vector<vk::DeviceMemory>  _depthImageMemories;       //!< Vulkan memories used for depth images
    std::vector<vk::ImageView>     _depthImageView;           //!< Vulkan depth image view
    std::vector<vk::Image>         _colorImage;               //!< Vulkan color image 
    std::vector<vk::DeviceMemory>  _colorImageMemories;       //!< Vulkan memories used for color images
    std::vector<vk::ImageView>     _colorImageView;           //!< Vulkan color image view
    std::vector<vk::Image>         _textureImage;             //!< Vulkan texture image 
    std::vector<uint32_t>          _textureImageMipmapLevel;  //!< mipmap levels
    std::vector<vk::DeviceMemory>  _textureImageMemories;     //!< Vulkan memories used for texture images
    std::vector<vk::ImageView>     _textureImageView;         //!< Vulkan texture image view
...
void CAppliction::initVulkan( void ) {
    ...
    if ( Verbose() ) {
        ...
        std::cout << std::endl;
        std::cout << "swap chain color space:        " << std::dec << (int)_surfaceFormat.colorSpace << std::endl;
        std::cout << "swap chain format:             " << std::dec << (int)_surfaceFormat.format << std::endl;
        std::cout << "swap chain presentation mode:  " << std::hex << (int)_presentMode << "h" << std::endl;
...
void CAppliction::cleanup( void ) {
    ...
    if ( _device) {
        ...
        for (auto indexBufferMemory : _indexBufferMemories) {
             _device.freeMemory(indexBufferMemory);
        }
        ...
        for (auto inFghtFence : _inFlightFences) {
             _device.destroyFence(inFghtFence);
        }
        _inFlightFences.clear();

        for (auto finishSemaphore : _renderFinishedSemaphores) {
             _device.destroySemaphore(finishSemaphore);
        }
    ...
    if ( _instance ) {
        ...
        _instance.destroyDebugReportCallbackEXT(_debubCallback);  
        _debubCallback = nullptr;

        //! The `vk::Instance` should be only destroyed right before the program exits. It can be destroyed in cleanup with the vkDestroyInstance function:
...
* \version 1.0
**********************************************************************/
vk::SampleCountFlagBits CAppliction::getMaxUsableSampleCount() {
    vk::PhysicalDeviceProperties physicalDeviceProperties = _physicalDevices.front()._physical_device.getProperties();
...
void CAppliction::createInstance( void ) {
    ...
    _validationLayers = vk::enumerateInstanceLayerProperties();
    ...
    if ( Verbose() ) {
        std::cout << _validationLayers.size() << " validation layers supported" << std::endl;
    ...
    _extensions = vk::enumerateInstanceExtensionProperties(nullptr);
    ...
    if ( Verbose() ) {
        std::cout << _extensions.size() << " extensions supported" << std::endl;
...
void CAppliction::setupDebugCallback( void ) {
    ...
    createInfo.setPfnCallback(&CAppliction::DebugCallback);
    createInfo.pUserData = this;
    
    _instance.createDebugReportCallbackEXT(&createInfo, nullptr, &_debubCallback);

    //! There are a lot more settings for the behavior of validation layers than just the flags specified in the `vk::DebugReportCallbackCreateInfoEXT` `struct`.
...
void CAppliction::createSurface( void ) {
    ...
    //! Although the `vk::SurfaceKHR` object and its usage is platform agnostic, its creation isn't because it depends on window system details.
    ...
    //!
    //! Because a window surface is a Vulkan object, it comes with a `vk::Win32SurfaceCreateInfoKHR` `struct` that needs to be filled in.
    ...
    */
    ...
    if ( glfwCreateWindowSurface(_instance, _wnd, nullptr, reinterpret_cast<VkSurfaceKHR *>(&_surface) ) != VK_SUCCESS )
...
void CAppliction::pickPhysicalDevice( void ) {
    ...
    //! After initializing the Vulkan library through a `vk::Instance` we need to look for and select a graphics card in the system that supports the features we need.
    ...
    for (const auto& device : devices) {
        ...
        for ( const auto& extension : availableExtensions )
            device_extensions.insert(extension.extensionName.data());
...
int CAppliction::rateDeviceSuitability( 
    vk::PhysicalDevice                device,            //!< device handle
    ...
    static const std::map<uint32_t, int> typescore_map {
...
void CAppliction::deviceInformation( 
    ...
    if ( Verbose() ) {
        ...
        static const std::map<uint32_t, std::string> typename_map {
...
TDeviceQueueInfo CAppliction::findQueueFamilies( 
    ...
    for ( int i=0; i<queueFamilies.size(); ++ i ) {
        ...
        if ( queueFamily.queueFlags & vk::QueueFlagBits::eGraphics )      info._graphics.push_back(i);
        if ( queueFamily.queueFlags & vk::QueueFlagBits::eCompute )       info._compute.push_back(i);
        if ( queueFamily.queueFlags & vk::QueueFlagBits::eTransfer )      info._transfer.push_back(i);
        if ( queueFamily.queueFlags & vk::QueueFlagBits::eSparseBinding ) info._sparse_binding.push_back(i);
...
void CAppliction::createLogicalDevice( void ) {
    ...
    vk::DeviceCreateInfo createInfo(
        ...
        queueCreateInfos,
        validationLayerParam,
        deviceExtensionParam,
        &deviceFeatures);

    _device = physicalDevice._physical_device.createDevice(createInfo);
    ...
    _graphicsQueue = _device.getQueue(physicalDevice._queue._graphics[0], 0);
    _presentQueue = _device.getQueue(physicalDevice._queue._graphics[0], 0);
}
...
* \version 1.0
**********************************************************************/
vk::SurfaceFormatKHR CAppliction::chooseSwapSurfaceFormat( 
    const std::vector<vk::SurfaceFormatKHR>& availableFormats //!< available formats of the selected device
    ) {
    ...
    //! The best case scenario is that the surface has no preferred format, which Vulkan indicates by only returning one `vk::SurfaceFormatKHR` entry which has its format member set to VK_FORMAT_UNDEFINED.
...
* \version 1.0
**********************************************************************/
vk::PresentModeKHR CAppliction::chooseSwapPresentMode( 
    const std::vector<vk::PresentModeKHR> availablePresentModes //!< available formats of the selected device
    ) {
...
* \version 1.0
**********************************************************************/
vk::Extent2D CAppliction::chooseSwapExtent( 
  const vk::SurfaceCapabilitiesKHR& capabilities //!< capabilities of the selected device
  ) {
...
void CAppliction::createSwapChain( void ) {
    ...
    std::vector<uint32_t> queueFamilyIndices{(uint32_t)indices._graphics[0], (uint32_t)indices._surface_support[0]};
    ...
    std::vector<uint32_t> emptyQueueFamilyIndices;
    ...
    (
        ...
        graphiscs_equal_presentation ? queueFamilyIndices : emptyQueueFamilyIndices,
...
void CAppliction::createRenderPass( void ) {
    ...
    //! Our array consists of a single `vk::AttachmentDescription`, so its index is 0.
    ...
    */
    ...
    std::vector<vk::AttachmentReference> resolveAttachments{colorAttachmentResolveRef};
    ...
    //! The render pass object can then be created by filling in the `vk::RenderPassCreateInfo` structure with an array of attachments and subpasses. 
    //! The `vk::AttachmentReference` objects reference attachments using the indices of this array.
...
void CAppliction::createImageViews( void ) {
    ...
    for (size_t i = 0; i < _swapChainImages.size(); i++) {
        createImageView( _swapChainImages[i], _surfaceFormat.format, vk::ImageAspectFlagBits::eColor, 1, _swapChainImageViews[i] );
    }
}
...
* \version 1.0
**********************************************************************/
vk::ShaderModule CAppliction::createShaderModule( 
    ...
    vk::ShaderModule shaderModule = _device.createShaderModule(createInfo);
    return shaderModule;
}
...
void CAppliction::createGraphicsPipeline( void ) {
    ...
    //! Assigning a shader module to either the vertex or fragment shader stage in the pipeline happens through a `vk::PipelineShaderStageCreateInfo` structure,
    ...
    vk::Viewport viewport(0.0f, 0.0f, (float)_extent.width, (float)_extent.height, 0.0f, 1.0f);
    ...
    //! Descriptor set layouts are specified in the pipeline layout object. Modify the `vk::PipelineLayoutCreateInfo` to reference the layout object.

    std::vector<vk::DescriptorSetLayout> layouts{_descriptorSetLayout};
    ...
    auto pipelines = _device.createGraphicsPipeline(vk::PipelineCache(), pipelineInfo);
    _graphicsPipeline = pipelines;
...
uint32_t CAppliction::findMemoryType( 
    vk::PhysicalDevice      device,     //!< I - the devices
    uint32_t              typeFilter, //!< I - type
    vk::MemoryPropertyFlags properties  //!< I - properties
    ) {
...
void CAppliction::createImage(
    ...
    vk::SampleCountFlagBits  numSamples, //!< I -
    vk::Format               format,     //!< I -
    vk::ImageTiling          tiling,     //!< I -
    vk::ImageUsageFlags      usage,      //!< I -
    vk::MemoryPropertyFlags  properties, //!< I -
    vk::Image               &image,      //!< O - texture image
    vk::DeviceMemory        &imageMemory //!< O - texture image memory
    ) {
    ...
    (
        ...
        findMemoryType(_physicalDevices.front()._physical_device, memRequirements.memoryTypeBits, properties)
    );
...
void CAppliction::createImageView(
    vk::Image             image,       //!< I -
    vk::Format            format,      //!< I -
    vk::ImageAspectFlags  aspectFlags, //!< I -
    uint32_t            mipLevels,   //!< I -
    vk::ImageView        &imageView )  //!< O -
{
    if ( !_device )
        throw CException("no logical vulkan device!");
...
vk::Format CAppliction::findSupportedFormat(const std::vector<vk::Format>& candidates, vk::ImageTiling tiling, vk::FormatFeatureFlags features) {
    ...
    for (vk::Format format : candidates) {
        vk::FormatProperties props = _physicalDevices.front()._physical_device.getFormatProperties(format);
...
void CAppliction::createColorResources(void) {
    ...
    vk::Format colorFormat = _surfaceFormat.format;

    _colorImage.push_back( VkImage() );
    _colorImageMemories.push_back( vk::DeviceMemory() );
    createImage(
    ...
    _colorImageView.push_back( vk::ImageView() );
    createImageView(_colorImage.back(), colorFormat, vk::ImageAspectFlagBits::eColor, 1, _colorImageView.back());
}
...
void CAppliction::createDepthResources( void )
{
    ...
    _depthImage.push_back( VkImage() );
    _depthImageMemories.push_back( vk::DeviceMemory() );
    createImage(
        _extent.width, _extent.height, 1, _msaaSamples,
        depthFormat,
        vk::ImageTiling::eOptimal,
    ...
    _depthImageView.push_back( vk::ImageView() );
    createImageView(_depthImage.back(), depthFormat, vk::ImageAspectFlagBits::eDepth, 1, _depthImageView.back());
}
...
void CAppliction::createTextureImage( void ) {
    ...
    _textureImage.push_back( VkImage() );
    _textureImageMemories.push_back( vk::DeviceMemory() );
    createImage(
        texWidth, texHeight, _textureImageMipmapLevel.back(), vk::SampleCountFlagBits::e1,
    ...
    transitionImageLayout(_textureImage.back(), vk::Format::eR8G8B8A8Unorm, vk::ImageLayout::eUndefined, vk::ImageLayout::eTransferDstOptimal, _textureImageMipmapLevel.back());
    ...
    if (_textureImageMipmapLevel.back() == 1)
        transitionImageLayout(_textureImage.back(), vk::Format::eR8G8B8A8Unorm, vk::ImageLayout::eTransferDstOptimal, vk::ImageLayout::eShaderReadOnlyOptimal, 1);

    _device.destroyBuffer(stagingBuffer);
    _device.freeMemory(stagingBufferMemory);

     if (_textureImageMipmapLevel.back() > 1)
         generateMipmaps(_textureImage.back(), vk::Format::eR8G8B8A8Srgb, texWidth, texHeight, _textureImageMipmapLevel.back());    
}
...
void CAppliction::generateMipmaps(vk::Image image, vk::Format imageFormat, int32_t texWidth, int32_t texHeight, uint32_t mipLevels) {
    ...
    vk::FormatProperties formatProperties = _physicalDevices.front()._physical_device.getFormatProperties(imageFormat);
...
void CAppliction::createTextureImageView( void ) {
    ...
    _textureImageView.push_back( vk::ImageView() );
    createImageView(_textureImage.back(), vk::Format::eR8G8B8A8Srgb, vk::ImageAspectFlagBits::eColor, _textureImageMipmapLevel.back(), _textureImageView.back());
}
...
void CAppliction::createTextureSampler( void )
{
    ...
    (
        ...
        static_cast<float>(_textureImageMipmapLevel.back()),
    ...
    _textureSampler.push_back(_device.createSampler(samplerInfo));
}
...
void CAppliction::createBuffer(
    vk::DeviceSize           size,        //!< I - size of the buffer
    vk::BufferUsageFlags     usage,       //!< I - buffer usage
    vk::MemoryPropertyFlags  properties,  //!< I - memory property
    vk::Buffer              &buffer,      //!< I - 
    ...
    std::vector<uint32_t> queueFamilyIndices;
    vk::BufferCreateInfo bufferInfo(vk::BufferCreateFlags{}, size, usage, vk::SharingMode::eExclusive, queueFamilyIndices);
...
void CAppliction::copyBuffer(
    vk::Buffer     srcBuffer, //!< I - source buffer
    vk::Buffer     dstBuffer, //!< I - target buffer
    vk::DeviceSize size       //!< I - buffer size
    ) {
...
void CAppliction::createVertexBuffer( void ) {
    ...
    _vertexBuffers.push_back( vk::Buffer() );
    _vertexBufferMemories.push_back( vk::DeviceMemory() );
    createBuffer(
        bufferSize,
...
void CAppliction::createIndexBuffer( void ) {
    ...
    _indexBuffers.push_back( vk::Buffer() );
    _indexBufferMemories.push_back( vk::DeviceMemory() );
    createBuffer(
        bufferSize,
...
void CAppliction::createDescriptorPool(void) {
    ...
    {
        vk::DescriptorPoolSize(vk::DescriptorType::eUniformBuffer, static_cast<uint32_t>(_swapChainImages.size())),
        vk::DescriptorPoolSize(vk::DescriptorType::eCombinedImageSampler, static_cast<uint32_t>(_swapChainImages.size())),
    };
...
void CAppliction::createDescriptorSets( void ) {
    ...
    for (size_t i = 0; i < _swapChainImages.size(); i++) {
        ...
        { 
            vk::DescriptorBufferInfo(_uniformBuffers[i], 0, sizeof(UniformBufferObject))
        };
        ...
        {
            vk::DescriptorImageInfo imageInfo(_textureSampler[j], _textureImageView[j],  vk::ImageLayout::eShaderReadOnlyOptimal);
...
void CAppliction::createCommandBuffers( void ) {
    ...
    for (size_t i = 0; i < _commandBuffers.size(); i++)
    {
        ...
        (
            ...
            vk::Rect2D(vk::Offset2D(0, 0), _extent),
            clearValues
        );
        ...
        std::vector<vk::Buffer> vertexBuffers = { _vertexBuffers.back() };
        ...
        _commandBuffers[i].bindIndexBuffer(_indexBuffers.back(), 0, index_type);
        ...
        _commandBuffers[i].bindDescriptorSets(vk::PipelineBindPoint::eGraphics, _pipelineLayout, 0, 1, &_descriptorSets[i], 0, nullptr);
...
void CAppliction::drawFrame( void ) {
    ...
    _device.waitForFences(1, &_inFlightFences[_currentFrame], VK_TRUE, std::numeric_limits<uint64_t>::max());
    ...
    auto result = _device.acquireNextImageKHR(_swapChain, std::numeric_limits<uint64_t>::max(), _imageAvailableSemaphores[_currentFrame], vk::Fence(), &imageIndex);
...
void CAppliction::updateUniformBuffer( uint32_t imageIndex )
{
    ...
    void* data = _device.mapMemory(_uniformBuffersMemories[imageIndex], 0, sizeof(ubo));
    memcpy(data, &ubo, sizeof(ubo));
    _device.unmapMemory(_uniformBuffersMemories[imageIndex]);
}
